...
export const CountryCard: React.FC<CountryCardProps> = ({ country, isLoading, onClose, theme }) => {
  ...
  // Famous people (safe even if types.ts not updated yet)
  ...
  // A consistent "bar" background so header/tabs never look hidden
  const barBg = isDark
  ...
  return (
    <AnimatePresence>
      {(country || isLoading) && (
        ...
        >
          {/* Background glow — force behind everything */}
          ...
          {/* Mobile Drag Handle */}
          <div
            className="relative z-20 w-full flex justify-center pt-3 pb-1 sm:hidden opacity-40 cursor-pointer"
            onClick={onClose}
            title="Close"
          >
          ...
          {/* Header (now always visible on top) */}
          <div
            className={`relative z-20 px-4 pb-2 pt-2 sm:p-4 flex items-center justify-between border-b shrink-0 backdrop-blur-xl
              ${barBg}`}
          >
            {isLoading ? (
              ...
              <div className="flex items-center gap-3 overflow-hidden">
                {country?.code && country.code !== '-99' ? (
                  ...
                  <span className="text-3xl shrink-0" role="img" aria-label="flag">
                    {country?.flagEmoji}
                  </span>
                )}
          ...
          {!isLoading && country && (
            <>
              {/* Tabs (now always visible, not “pushed up”) */}
              <div
                className={`relative z-20 flex border-b shrink-0 backdrop-blur-xl
                  ${barBg}`}
              >
                <button
                  onClick={() => setActiveTab('info')}
                  className={`flex-1 py-3 text-sm font-semibold flex items-center justify-center gap-2 transition-colors relative
                    ${activeTab === 'info' ? 'text-blue-500' : 'opacity-80 hover:opacity-100'}`}
                >
                  <Info size={16} /> Info
                  {activeTab === 'info' && (
                ...
                <button
                  ...
                  className={`flex-1 py-3 text-sm font-semibold flex items-center justify-center gap-2 transition-colors relative
                    ${activeTab === 'chat' ? 'text-blue-500' : 'opacity-80 hover:opacity-100'}`}
                >
              ...
              <div className="relative z-10 flex-1 overflow-hidden bg-transparent">
                        ...
                        {country.touristSites.map((site, i) => (
                          <motion.div
                            ...
                            className={`group flex items-start gap-2 p-3 rounded-2xl border transition-all
                              ${isDark ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-white border-gray-200/60 hover:bg-gray-50'}`}
                          >
                ...
                {activeTab === 'chat' && (
                  ...
                  >
                    ...
                    {/* Chat composer: add safe-area padding so it “sits comfortably” */}
                    <div
                      className={`p-3 border-t shrink-0 backdrop-blur-xl
                        ${isDark ? 'border-white/10 bg-slate-900/60' : 'border-gray-100 bg-white/70'}
